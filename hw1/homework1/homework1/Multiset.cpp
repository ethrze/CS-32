...
// "turmeric" is 2.
{
    ...
    for (int i = 0; i < DEFAULT_MAX_ITEMS; i++)                     // set[] loop
    {
        //        if (set[i].element() != "")                               // if it isn't an empty string, it's a candidate for uniqueness
        //        {
        int track = 0;
        for (int j = 0; j < theCount; j++)
        {
            if (set[i].element() == uniqArr[j])
            {
        ...
        if (track == 0)
        {
            uniqArr[theCount] = set[i].element();
...
// Return the number of instances removed, which will be 1 or 0.
{
    ...
    {
        if (set[i].element() == value)
        {
            for (int j = i; j < DEFAULT_MAX_ITEMS; j++)
            {
                set[j].setEl(set[j+1].element());
            }
            setSize--;
            return 1;
        }
...
// (See below for details about this function.)
{
    ...
    if (i >= 0 && i < this->uniqueSize()) {
        ...
        for (int k = 0; k < DEFAULT_MAX_ITEMS; k++)                 // set[] loop
        {
            ...
            if (track == 0)
            {
                uniqArr[theCount] = set[k].element();
                theCount++;     // if there aren't any matches, it's unique
            }
        }
